...
%post
    ...
    R --slave -e 'library("BiocManager"); library("packrat"); instp <- installed.packages(); instp <- unique(instp[,1]); installpack <- c("devtools", "org.Hs.eg.db",  "GenomicRanges", "tidyverse", "bio3d", "plyr", "pheatmap", "data.table",  "Biobase",  "ensembldb"); allpack <- packrat:::recursivePackageDependencies(installpack, lib.loc=.libPaths()); allpack <- allpack[! allpack %in% uinstp]; BiocManager::install(c(installpack, allpack), dependencies=FALSE, update=FALSE, ask=FALSE); BiocManager::install(c("ensemblVEP", "SNPchip", "QDNAseq", "QDNAseq.hg19", "customProDB", "EnsDb.Hsapiens.v75"), dependencies=TRUE, update=FALSE, ask=FALSE)'
